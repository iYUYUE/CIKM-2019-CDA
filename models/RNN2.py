...
class SimpleRNN(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.rnn = nn.LSTM(hidden_size, hidden_size, 3, dropout=0.05)
...
for epoch in range(n_epochs):
    ...
    for iter in range(n_iters):
        _inputs = sample(50)
        inputs = Variable(torch.from_numpy(_inputs[:-1]).float()).cuda()
        targets = Variable(torch.from_numpy(_inputs[1:]).float()).cuda()
