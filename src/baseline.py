...
if sys.argv[1] == 'train':
    ...
    for filename in os.listdir(directory):
        ...
        # RNN.py teacher_forcing_ratio = 0.5 entitied_msdialog.csv ./model/kzwrrwwrtq/
        # baseline.py entitied_msdialog.csv ./model/ruyubbugqe/
        # RNN.py teacher_forcing_ratio = 0.0 entitied_msdialog.csv ./model/zcjruetwdo/
        # RNN.py teacher_forcing_ratio = 0.5 collapsed_msdialog.csv ./model/rbnuepuapa/
        # baseline.py collapsed_msdialog.csv ./model/jznqsfjeyt/
        for i in range(n_iters):
            inputs = Variable(torch.from_numpy(np.array([str2vector(bow_index, sent, True) for sent in X_test[i]])).float()).to(device)
            targets = Variable(torch.from_numpy(np.array([str2vector(ms_tags, sent, False) for sent in y_test[i]])).float()).to(device)
            hidden = model.initHidden()
